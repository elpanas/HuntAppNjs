...
async function generateQrPdf(idg) {  
    ...
    if(!fs.existsSync(tmpqrc)) fs.mkdir(tmpqrc, {recursive: true});
    if(!fs.existsSync(tmppdf)) fs.mkdir(tmppdf, {recursive: true});
    if(!fs.existsSync(dirpdf)) fs.mkdir(dirpdf, {recursive: true});    
